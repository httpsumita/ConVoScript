...
def registerPage(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            user=form.save(commit=False) # gives ability to clean data after user has entered
...
def createRoom(request):
    ...
    if request.method =='POST':
        ...
        topic, created =Topic.objects.get_or_create(name=topic_name) #returns object to topics if already exists and creeated is false or vice versa
        Room.objects.create(
            host=request.user,
            topic=topic,
